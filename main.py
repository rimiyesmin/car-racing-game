...
# Road boundaries (based on image layout)
LEFT_BOUND = 100
RIGHT_BOUND = 420
0  # Road ends at 420px
...
def start_game(difficulty):
    ...
    player_y = HEIGHT - 120
    enemy_x = random.randint(LEFT_BOUND, RIGHT_BOUND - CAR_WIDTH)
    enemy_y = -CAR_HEIGHT
    score = 0
    ...
    while running:
        ...
        # Controls with road boundary
        keys = pygame.key.get_pressed()
        if keys[pygame.K_LEFT] and player_x > LEFT_BOUND:
            player_x -= 5
        if keys[pygame.K_RIGHT] and player_x < RIGHT_BOUND - CAR_WIDTH:
            player_x += 5
